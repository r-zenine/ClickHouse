...
bool sliceHasImpl(const FirstSliceType & first, const SecondSliceType & second, const UInt8 * first_null_map, const UInt8 * second_null_map) noexcept
{
    if constexpr (search_type == ArraySearchType::SubSeq)
        return sliceHasImplSubSeq<FirstSliceType, SecondSliceType, isEqual, isEqualSecond>(first, second, first_null_map, second_null_map);
    else
...
bool sliceHasImplSubSeq(const FirstSliceType & first, const SecondSliceType & second, const UInt8 * first_null_map, const UInt8 * second_null_map) noexcept
{
    ...
    const auto aux = buildAux<SecondSliceType, isEqualUnary>(second, second_null_map);
...
UInt8 * buildAux(const SliceType & pattern, const UInt8 * pattern_null_map) noexcept
{
    auto aux = new UInt8[pattern.size];
